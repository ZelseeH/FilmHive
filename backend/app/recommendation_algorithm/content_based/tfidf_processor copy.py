...
# Inicjalizacja NLTK (raz, jeśli potrzeba)
try:
    skrypt_nltk.data.find("stemmers/stemmer_data/polish.json")  # Dla Snowball
except LookupError:
    skrypt_nltk.download("punkt")  # Podstawowy, jeśli brak
...
class TFIDFProcessor:
    def __init__(self, use_snowball: bool = True):
        self.vectorizer = None
        self.feature_names = None
        if use_snowball:
            self.stemmer = SnowballStemmer(
                "polish"
            )  # Lepszy dla PL [17], np. "technologia" -> "technolog"
        else:
            self.stemmer = None  # Fallback do custom
    ...
    def preprocess_text(self, text: str) -> str:
        ...
        for word in words:
            if len(word) <= 2:
                ...
                continue
            if self.stemmer:
                stemmed_word = self.stemmer.stem(
                    word
                )  # Snowball dla PL (np. "sny" -> "sn", "śpiącego" -> "śpi")
            else:
                stemmed_word = self._simple_polish_stemming(word)  # Fallback
            stemmed_words.append(stemmed_word)

        return " ".join(stemmed_words)

    def _simple_polish_stemming(self, word: str) -> str:
        """Fallback custom stemming dla polskiego (ulepszone endings)"""
    ...
    # Pozostałe metody bez zmian (get_feature_weights, get_document_similarity, get_top_features_by_class, get_vectorizer_info)
    ...
    def get_vectorizer_info(self) -> Dict:
        ...
        try:
            info = {
                ...
                "stemmer": "Snowball (Polish)" if self.stemmer else "Custom",
            }
