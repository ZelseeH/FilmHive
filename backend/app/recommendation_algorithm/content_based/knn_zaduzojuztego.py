...
class KNNRecommender:
    ...
    3. Apply adaptive weights (boost genres/actors/directors with patterns)
    4. Compute weighted cosine similarity between user profile and candidates
    ...
    def fit(
        ...
        Args:
            ...
            negative_ratings: DataFrame z negatywnymi ocenami (≤4)
            negative_features: DataFrame z cechami negatywnie ocenionych filmów
        ...
        if not negative_features.empty and len(negative_ratings) >= 2:
            ...
            if list(negative_features_only.columns) == self.feature_names:
                ...
                self.user_profile = np.clip(self.user_profile, 0, None)
