...
class DataPreprocessor:
        ...
        data = [
            {
                ...
                "title": result.title,
                "description": result.description or "",  # Fallback empty
        ...
        data = [
            {
                "movie_id": result.movie_id,
                "title": result.title,
                "description": result.description or "",  # Fallback
                "release_date": result.release_date,
                "duration_minutes": result.duration_minutes,
                "country": result.country,
                "original_language": result.original_language,
    ...
    def align_features(
        ...
        # FIX PerformanceWarning: Add missing cols via pd.DataFrame constructor (not loop)
        # Create dict of missing cols with default 0.0
        missing_rated = {
        ...
        # Add missing cols at once (no fragmentation)
        if missing_rated:
        ...
        # Reorder to standard: movie_id + sorted features
    ...
    def prepare_textual_features(self, movies_df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Filter out too short descriptions (noise)
        valid_indices = [
        ...
        try:
            ...
            # Merge back to full movies_df (fill 0 for skipped)
            full_tfidf = (
                movies_df[["movie_id"]]
    ...
    def get_movie_descriptions(self, movie_ids: List[int]) -> pd.DataFrame:
        ...
        try:
            movies = (
                self.db.query(Movie.movie_id, Movie.description)
                .filter(Movie.movie_id.in_(movie_ids))
                .all()
            )
            data = [
                {"movie_id": m.movie_id, "description": m.description or ""}
                for m in movies
            ]
            df = pd.DataFrame(data)
            self.logger.info(f"Pobrano {len(df)} opisów dla NB")
            return df
        except Exception as e:
            self.logger.error(f"Error get_movie_descriptions: {e}")
